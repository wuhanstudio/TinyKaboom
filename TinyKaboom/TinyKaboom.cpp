    ...
    for (size_t j = 0; j < height; j++) { 
        for (size_t i = 0; i < width; i++) {
            ...
            if (sphere_trace(Vec3f(0, 0, 3), Vec3f(dir_x, dir_y, dir_z).normalize(), hit)) { // the camera is placed to (0,0,3) and it looks along the -z axis
                ...
                Vec3f color = palette_fire((-.2 + noise_level) * 2);
                pixels[index++] = int(color.x * 255) * light_intensity; // *displacement;
                pixels[index++] = int(color.y * 255) * light_intensity; // *displacement;
                pixels[index++] = int(color.z * 255) * light_intensity; // *displacement;
            }
            else {
                // background color
                pixels[index++] = int(0.2*255);
                pixels[index++] = int(0.7*255);
                pixels[index++] = int(0.8*255);
            } 
        }
    }
